...
exports.flattenKeys =
  exports.generateTypesContent =
  exports.sortNestedKeys =
  exports.mergeNestedKeys =
  exports.updateLangFiles =
  exports.processLangFiles =
...
const processLangFiles = (langDir, allKeys) => {
  ...
  for (const file of files) {
    ...
    const interfaceName = (0, stringOps_1.toPascal)(
      path_1.default.basename(file, '.json'),
    );
...
const updateLangFiles = (langDir, allKeys) => {
  ...
  for (const file of files) {
    ...
    const updatedJsonContent = (0, exports.mergeNestedKeys)(
      jsonContent,
      allLangKeys,
    );
...
const generateTypesContent = (allKeys, typesPath) => {
  ...
  for (const [interfaceName, keys] of Object.entries(allKeys)) {
    ...
    const typeFile = path_1.default.join(
      typesPath,
      `_${interfaceName.toLowerCase()}.ts`,
    );
...
`;
    ...
    logger_1.default.info(
      `✅ Generated types for ${interfaceName} -> ${path_1.default.relative(process.cwd(), typeFile)}`,
    );
...
${combinedKeysContent}
  `;
  fs_1.default.writeFileSync(
    path_1.default.join(typesPath, 'index.ts'),
    indexContent,
  ); // No "_" prefix for index.ts
