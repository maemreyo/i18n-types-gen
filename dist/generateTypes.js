 ...
 */
const generateTypes = async ({ localePath = path_1.default.resolve(process.cwd(), '_locales'), typesPath = path_1.default.resolve(process.cwd(), '_types/i18n'), autoAddMissingKeys = true, dryRun = false, }) => {
    // Determine the versioned output directory
    const versionedTypesDir = path_1.default.join(typesPath, '..', (0, versioning_1.getNextVersionedDir)(path_1.default.join(typesPath, '..'), path_1.default.basename(typesPath)));
    ...
    if (!dryRun && !fs_1.default.existsSync(versionedTypesDir)) {
        ...
        logger_1.default.info(`📁 Created versioned types directory: ${path_1.default.relative(process.cwd(), versionedTypesDir)}`);
    }
    try {
        ...
        Object.keys(allKeys).forEach((interfaceName) => {
            allKeys[interfaceName] = (0, fileOps_1.flattenKeys)(allKeys[interfaceName]);
        });
